...
const commandsDir = join(import.meta.dir, '../packages/cli/src/commands')
const files = await findCommandFiles(commandsDir)

const tools: ToolDef[] = []
const skipFiles = ['proxy', 'plugin', 'status', 'mcp', 'profile', 'render', 'patch']
...
await Bun.write('dist/mcp-tools.json', JSON.stringify(tools))
