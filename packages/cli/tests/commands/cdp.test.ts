...
describe('CDP commands', () => {
  ...
  describe('me', () => {
    test('returns current user', async () => {
      const result = await run('me --json') as { id: string; handle?: string; name?: string; email?: string }
  ...
  describe('version list', () => {
    test('returns versions array', async () => {
      const result = await run('version list --limit 5 --json') as { id: string; created_at: string; user: { handle: string } }[]
