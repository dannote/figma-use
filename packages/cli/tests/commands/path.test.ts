...
async function createVector(path: string, parent: string): Promise<string> {
  const group = await run(`create vector --x 10 --y 10 --path "${path}" --fill "#FF0000" --parent ${parent} --json`) as any
  trackNode(group.id)
...
describe('path', () => {
  ...
  beforeAll(async () => {
    ...
    const frame = await run('create frame --x 0 --y 0 --width 400 --height 300 --name "Path Tests" --json') as { id: string }
  ...
  test('get returns path data', async () => {
    const result = await run(`path get ${vectorId} --json`) as { paths: Array<{ windingRule: string; data: string }> }
  ...
  test('move translates path points', async () => {
    ...
    const result = await run(`path move ${vectorId} --dx 10 --dy 20 --json`) as { paths: Array<{ data: string }> }
  ...
  test('scale changes path size', async () => {
    ...
    const result = await run(`path scale ${vid} --factor 2 --json`) as { paths: Array<{ data: string }> }
  ...
  test('flip x mirrors horizontally', async () => {
    ...
    const result = await run(`path flip ${vid} --axis x --json`) as { paths: Array<{ data: string }> }
  ...
  test('flip y mirrors vertically', async () => {
    ...
    const result = await run(`path flip ${vid} --axis y --json`) as { paths: Array<{ data: string }> }
    expect(result.paths[0].data).toContain('-100')
  })
})
...
describe('node bounds', () => {
  ...
  beforeAll(async () => {
    ...
    const frame = await run('create frame --x 100 --y 200 --width 400 --height 300 --name "Bounds Tests" --json') as { id: string }
    testFrameId = frame.id
    trackNode(testFrameId)

    const rect = await run(`create rect --x 50 --y 50 --width 100 --height 80 --fill "#AAAAAA" --parent "${testFrameId}" --json`) as any
    rectId = rect.id
    trackNode(rectId)
  })
