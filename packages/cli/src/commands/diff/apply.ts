...
export default defineCommand({
  ...
  args: {
    ...
    dryRun: { type: 'boolean', description: 'Show what would be changed without applying' },
    ...
    try {
      ...
      for (const patch of patches) {
        ...
        if (isCreate) {
          ...
          if (args.dryRun) {
            results.push({ nodeId: 'new', path, status: 'created', changes: newProps as Record<string, unknown> })
            continue
          }
        ...
        try {
          currentNode = await sendCommand('get-node-info', { id: nodeId }) as Record<string, unknown>
        } catch (e) {
        ...
        if (args.dryRun) {
          results.push({ nodeId, path, status: 'applied', changes: changes as Record<string, unknown> })
          continue
        }
        ...
        try {
          ...
          if (changes.stroke) {
            await sendCommand('set-stroke-color', { id: nodeId, color: changes.stroke, weight: changes.strokeWeight })
          }
          ...
          if (changes.size) {
            await sendCommand('resize-node', { id: nodeId, width: changes.size[0], height: changes.size[1] })
          }
          if (changes.pos) {
      ...
      if (args.json) {
        ...
        for (const r of results) {
          const icon = r.status === 'applied' ? '✓' : 
                       ...
                       r.status === 'deleted' ? '-' :
                       r.status === 'skipped' ? '○' : '✗'
          const color = r.status === 'applied' || r.status === 'created' || r.status === 'deleted' ? ok : 
                        r.status === 'skipped' ? ((s: string) => s) : fail
        ...
        const applied = results.filter(r => r.status === 'applied' || r.status === 'created' || r.status === 'deleted').length
