...
async function findCommandFiles(dir: string, prefix = ''): Promise<{ path: string; name: string }[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      files.push(...await findCommandFiles(fullPath, prefix ? `${prefix}_${entry.name}` : entry.name))
...
function parseCommandFile(content: string, name: string): ToolDef | null {
  ...
  function visit(node: ts.Node) {
        ...
        for (const prop of arg.properties) {
          ...
          if (propName === 'meta' && ts.isObjectLiteralExpression(prop.initializer)) {
            for (const metaProp of prop.initializer.properties) {
              if (ts.isPropertyAssignment(metaProp) && metaProp.name.getText(sourceFile) === 'description') {
    ...
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === 'sendCommand') {
